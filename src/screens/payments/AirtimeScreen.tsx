...
import { C, AIRTIME_PROVIDERS } from '../../constants';
...
export default function AirtimeScreen({ navigation }: any) {
  ...
  const [provider, setProvider] = useState<typeof AIRTIME_PROVIDERS[0] | null>(null);
  ...
  const buy = async () => {
    if (!provider) { Alert.alert('Select Network', 'Please select a network provider.'); return; }
    if (!phone.trim() || !/^(07|01)\d{8}$/.test(phone)) { Alert.alert('Invalid Phone', 'Enter a valid Kenyan phone number.'); return; }
    if (!amount || Number(amount) < 5) { Alert.alert('Invalid Amount', 'Minimum airtime is KES 5.'); return; }
    ...
    try {
      ...
      const tx = {
        ...
        description: `${provider.name} Airtime — ${phone}`,
  ...
  const reset = () => { setDone(false); setProvider(null); setPhone(''); setAmount(''); setPin(''); };

  if (done) return (
    <SuccessScreen
      title="Airtime Sent!"
      subtitle={`${provider?.name} — ${phone}`}
        ...
        <ScrollView style={s.content} keyboardShouldPersistTaps="handled">
          ...
          <View style={{ flexDirection: 'row', gap: 10, marginBottom: 20 }}>
            {AIRTIME_PROVIDERS.map(p => {
              ...
              return (
                <TouchableOpacity key={p.id} style={[s.netCard, active && { backgroundColor: p.color, borderColor: p.color }]} onPress={() => setProvider(p)}>
                  <Text style={[s.netName, active && { color: '#fff' }]}>{p.name}</Text>
                </TouchableOpacity>
              );
            })}
          </View>

          <F label="Phone Number" value={phone} set={setPhone} ph="07XX XXX XXX" kb="phone-pad" />

          {/* Quick amounts */}
          {provider && (
            <>
              <Text style={s.label}>Quick Amounts</Text>
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 16 }}>
                {provider.amounts.map(a => (
                  <TouchableOpacity key={a} style={[s.amtChip, amount === String(a) && s.amtChipActive]} onPress={() => setAmount(String(a))}>
                    <Text style={[s.amtText, amount === String(a) && { color: '#fff' }]}>KES {a}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </>
          ...
          {loading
            ...
            : (
              <TouchableOpacity style={[s.btn, !provider && { opacity: 0.5 }]} onPress={buy} disabled={!provider} activeOpacity={0.85}>
                <LinearGradient colors={['#C8102E', '#9A0B23']} style={s.btnGrad}>
                  <Text style={s.btnText}>Buy Airtime</Text>
                </LinearGradient>
              </TouchableOpacity>
            )
          }
...
const s = StyleSheet.create({
  ...
  netCard:     { flex: 1, backgroundColor: '#fff', borderRadius: 12, padding: 14, alignItems: 'center', borderWidth: 1.5, borderColor: C.border },
  netName:     { fontSize: 14, fontWeight: '700', color: C.text },
  amtChip:     { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20, borderWidth: 1.5, borderColor: C.border, backgroundColor: '#fff' },
  amtChipActive: { backgroundColor: C.primary, borderColor: C.primary },
  amtText:     { fontSize: 13, fontWeight: '600', color: C.textSub },
